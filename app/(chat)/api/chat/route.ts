...
export async function POST(request: Request) {
  ...
  // Allow anonymous usage when database is not configured
  ...
  // Try to get chat from database, but don't fail if database is unavailable
  let chat;
  try {
    chat = await getChatById({ id });
    
    if (!chat && process.env.POSTGRES_URL) {
      const title = await generateTitleFromUserMessage({ message: userMessage, modelApiKey });
      await saveChat({ id, userId, title });
    }
  } catch (error) {
    console.warn('Database operations failed, continuing without persistence:', error);
  }

  const userMessageId = generateUUID();

  // Try to save messages, but don't fail if database is unavailable
  try {
    ...
    console.warn('Failed to save messages to database, continuing:', error);
  }
    ...
    execute: async (dataStream) => {
      ...
      const result = streamText({
        ...
        onFinish: async ({ response }) => {
          ...
          // save the response (only if database is available)
          if (userId && userId !== 'anonymous-user' && process.env.POSTGRES_URL) {
            try {
            ...
            console.log('Database not configured or anonymous user, skipping message save');
          }
        },
      });
